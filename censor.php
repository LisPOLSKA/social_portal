...
<body>
                ...
                <ul class="navbar-nav" style="align-items:center">
                    ...
                    <li class="nav-item dropdown h5 active">
                        <a class="nav-link dropdown-toggle text-light" href="#" id="navbarDropdown"
                            data-bs-toggle="dropdown" role="button" aria-expanded="false">Not Censored</a>
                        <ul class="dropdown-menu">
                            <?php
                                ...
                                if($polaczenie->connect_errno!=0)
                                {
                                    ...
                                    if($rezultat = $polaczenie->query(sprintf("SELECT * FROM art_censor ORDER BY add_time DESC"))){
                                        $l_od = $rezultat->num_rows;
                                        while($row = mysqli_fetch_array($rezultat)){
                                            $tem = $row['id'];
                                            $nag = $row['nag'];
                                            echo('<li>
                                            <a class="dropdown-item" href="#'.$tem.'">'.$nag.'</a>');
                                        }
                                    }
                                    else{
                                        throw new Exception($polaczenie->error);
    ...
    <div class="modal fade" tabindex="-1" id="achtung">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" style="color:red;">Uwaga!!</h5>
                </div>
                <div class="modal-body">
                    <p>Wchodzisz do kategorii gdzie mogą wystąpić treści niecenzuralne, wulgarne oraz uznawane za
                        odrażające!</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-danger" data-bs-dismiss="modal" id="okzak">Wchodzę</button>
                    <a href="index.php" class="btn btn-success">Wyjdź</a>
                </div>
            </div>
        </div>
    </div>
    <main class="row mx-0 fs-4" id="praca">
            ...
            if($polaczenie->connect_errno!=0)
            {
                ...
                if($rezultat = $polaczenie->query(sprintf("SELECT * FROM art_censor ORDER BY add_time DESC"))){
                    ...
                    while($row = mysqli_fetch_array($rezultat)){
                        ...
                        $zaw = $row['zaw'];
                        $aut = $row['aut'];
                        if($rezu = $polaczenie->query(sprintf("SELECT id, aut, zaw FROM kom_censor WHERE id_art='$id' ORDER BY add_date DESC LIMIT 2"))){
                            if($re = $polaczenie->query(sprintf("SELECT id FROM kom_censor WHERE id_art='$id'")))
                            $l_odp = $re->num_rows;
                        }
                        else{
                            throw new Exception($polaczenie->error);
                        ...
                        echo('<form action="sex1.php" method="post" class="text-center">
                                <div class="row mb-3">
                                    ...
                                    <div class="d-none">
                                        ...
                                        <input type="text" readonly name="art" id="art" value="cen">
                                    </div>
                                </div>
                            </form>');
                        }
                        ...
                        <a href="sex2.php?data='.$id.'&art=censor">czytaj więcej</a>
                        </div>
                        </div>
                        </div>');}
                        echo('
    ...
    var myModal = new bootstrap.Modal(document.getElementById('achtung'), {
        keyboard: false
    })
    myModal.show();
    document.getElementById('praca').style.filter = "blur(50px)";
    document.getElementById('okzak').addEventListener("click", ok);

    function ok() {
        document.getElementById('praca').style.filter = "blur(0px)";
    }
    </script>
</body>

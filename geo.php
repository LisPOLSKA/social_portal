                ...
                <ul class="navbar-nav" style="align-items:center">
                    ...
                    <li class="nav-item dropdown h5 active">
                        <a class="nav-link dropdown-toggle text-light" href="#" id="navbarDropdown"
                            data-bs-toggle="dropdown" role="button" aria-expanded="false">AI</a>
                        <ul class="dropdown-menu">
                            <?php
                                ...
                                if($polaczenie->connect_errno!=0)
                                {
                                    ...
                                    if($rezultat = $polaczenie->query(sprintf("SELECT id, nag FROM art_akcja ORDER BY add_time DESC"))){
    ...
    <main class="row mx-0 fs-4">
        <div class="container col-sm-12">
            ...
            if($polaczenie->connect_errno!=0)
            {
                ...
                if($rezultat = $polaczenie->query(sprintf("SELECT * FROM art_akcja ORDER BY add_time DESC"))){
                    $l_od = $rezultat->num_rows;
                    $l_odp;
                    while($row = mysqli_fetch_array($rezultat)){
                        $id = $row['id'];
                        ...
                        $zaw = $row['zaw'];
                        $aut = $row['aut'];
                        if($rezu = $polaczenie->query(sprintf("SELECT id, aut, zaw FROM kom_geo WHERE id_art='$id' ORDER BY add_date DESC LIMIT 2"))){
                            if($re = $polaczenie->query(sprintf("SELECT id FROM kom_geo WHERE id_art='$id'")))
                            $l_odp = $re->num_rows;
                        }
                        else{
                            throw new Exception($polaczenie->error);
                        ...
                        echo('<form action="sex1.php" method="post" class="text-center">
                                <div class="row mb-3">
                                    ...
                                    <div class="d-none">
                                        ...
                                        <input type="text" readonly name="art" id="art" value="akcja">
                                    </div>
                                </div>
                            </form>');
                        }
                        ...
                        <a href="sex2.php?data='.$id.'&art=akcja">czytaj więcej</a>
                        </div>
                        </div>
                        </div>');}
                        echo('
